...
500     do j=2,(y_node_number-1)
    ...
    do j=1,y_node_number
        T2(1,j)=T2(2,j)+q_boundary*dx_face_node_E(2)/k_boundary
    end do
    ...
    do i=1,x_node_number
        T2(i,1)=(h*dy_face_node_N(2)/k_boundary*T_f+T2(i,2))/(h*dy_face_node_N(2)/k_boundary+1.0)
    end do

    do i=1,x_node_number
        T2(i,y_node_number)=(h*dy_face_node_S(y_node_number)/k_boundary*T_f+T2(i,y_node_number-1))/(h*dy_face_node_S(y_node_number)/k_boundary+1.0)
    end do
